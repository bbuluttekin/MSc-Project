...
Continuous integration (CI) is a workflow strategy that helps ensure everyone's changes will integrate with the current version of the project in the typical software engineering team. 
This allows members of the team find merge conflicts, and increase overall reliability of the software. 
Often times, a team will apply CI with automated testing using a private server or CI service. 
Anytime a developer adds new work or commits to a branch, the server will automatically trigger a build and test the code to determine whether it works and can be integrated with the rest of code on the development branch as well as release branch. 
...
With introducing build and test information to commits on all of branches, CI makes continuous delivery possible along with the related process called continuous deployment. 
...
Combined with CI, continuous delivery lets you develop projects with modular code base that can be integrated to rest of the project with ease.
Even though the benefits of using CI/CD pipelines are more prominent in the software teams, integration automated testing will help even individual projects such as this by reducing time for debugging.
