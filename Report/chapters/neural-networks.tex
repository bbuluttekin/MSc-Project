...
For example lets suppose \(\hat{y} = \sigma(z)\) and \(z = wx + b\). Then chain rule states that

\begin{equation} \label{eq:chain}
  \frac{\partial{\hat{y}}}{\partial{x}} = \frac{\partial{\hat{y}}}{\partial{\sigma}} . \frac{\partial{\sigma}}{\partial{x}}
\end{equation}

It is clear that the gradient of \textbf{\textit{x}} is predicated on the gradient of the $\sigma$ is not being zero.
...
Because of consistent properties of the non-saturating functions, it is always better to use such activation functions for hidden layers to ensure the gradient flow is faster and Network will converge faster to end result.
